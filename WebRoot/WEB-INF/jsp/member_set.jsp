		...
		<div class="member_per">
			<div class="panel clearfix">
				<div class="panel-info clearfix">
					...
					<div class="name-job">
						<h4 class="myname"> ${sessionScope.user.user_name }</h4>
						<p class="job-title"> ${sessionScope.user.user_email } </p>
					</div>
				</div>
			</div>
		</div>
        ...
        <div class="setting-right-wrap wrap-boxes settings">
            <div id="setting-profile" class="setting-wrap setting-profile">
                ...
                <div class="wlfg-wrap">
                    ...
                    <label>
                        <input type="radio" name="sex" value="1" hidefocus="true" autocomplete="off" ${man }/>男
                    </label>
                    <label>
                        <input type="radio" name="sex" value="2" hidefocus="true" autocomplete="off" ${woman }/>女
                    </label>
                </div>
                <div class="wlfg-wrap">
                    <label class="label-name" for="job">手机号码</label>
                    <input type="text" id="phone" class="rlf-input" placeholder="请输入手机号码"  value="${sessionScope.user.user_phone }"/>
                </div>
                 <div class="wlfg-wrap">
                    ...
                    <select id="question" class="rlf-select">
                    	...
                    	<c:set var="selectedd">
                    		<c:if test="${question.question_id==sessionScope.user.question_id }">
                    		selected="selected"
                    		</c:if>
                    	</c:set>
                    	<option value="${question.question_id }" ${selectedd }>${question.question_title }</option>
                    	</c:forEach>
                    </select>
                </div>
                <div class="wlfg-wrap">
                    <label class="label-name" for="job">密保答案</label>
                    <input type="text" id="phone" class="rlf-input" placeholder="请输入密保答案" value="${sessionScope.user.question_answer }" />
                </div>
                <div class="wlfg-wrap">
